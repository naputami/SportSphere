...
export const CommunityCard = ({ name, city, sportType, isPrivate, imageTitle, communityId }) => {
  return (
    <div className="card card-compact bg-base-100 w-full shadow-xl static">
      <figure className="md:h-40 lg:h-48">
        <Image src={`${process.env.R2_PUBLIC_URL}/communities/${communityId}/${imageTitle}`} width={500} height={500} alt="Picture of the author" />
      </figure>
      <div className="card-body gap-0">
        <div className="flex justify-between">
          <Link href={`/community/${communityId}`} className="card-title m-0 hover:underline">
            {name}
          </Link>
